...
public class BidiPBAReference
{
    ...
    public const sbyte n = 0;
    public const sbyte o = 1;
    public const sbyte c = 2;

    sbyte sos; // direction corresponding to start of sequence
    ...
    public class BracketPair : IComparable<BracketPair>
    {
        ...
        {
            if (other is BracketPair otherPair)
            {
    ...
    LinkedList<int> openers; // list of positions for opening brackets
    ...
    {
        SortedSet<BidiPBAReference.BracketPair> tempPositions = new SortedSet<BidiPBAReference.BracketPair>();
        foreach (BidiPBAReference.BracketPair pair in pairPositions)
        {
            tempPositions.Add(new BidiPBAReference.BracketPair(indexes[pair.getOpener()], indexes[pair.getCloser()]));
        }
        var sb = new StringBuilder();
        sb.Append("[");
        foreach (var pair in tempPositions)
        {
    ...
    private sbyte[] initialCodes; // direction bidi codes initially assigned to the original string
    public sbyte[] codesIsolatedRun; // directional bidi codes for an isolated run
	 ...
	 * @param index
	 */
    private void removeHead(LinkedList<int> list, LinkedListNode<int> removeToNode)
    {
        var node = list.First;
        while (node != null)
        {
	 ...
	 */
    private void locateBrackets(sbyte[] pairTypes, int[] pairValues)
    {
        openers = new LinkedList<int>();
        pairPositions = new SortedSet<BracketPair>();
        ...
        for (int ich = 0; ich < indexes.Length; ich++)
        {
            ...
            switch (pairTypes[indexes[ich]])
            {
                ...
                case c:
                    ...
                    var node = openers.First;
                    while (node != null)
                    {
                        int opener = node.Value;
                        if (matchOpener(pairValues, opener, ich))
                        {
                            // if the opener matches, add nested pair to the ordered list
                            pairPositions.Add(new BracketPair(opener, (int)ich));
                            // remove up to and including matched opener
                            removeHead(openers, node);
                            break;
                        }
                        node = node.Next;
                    }
	 ...
	 */
    private sbyte getStrongTypeN0(int ich)
    {
	 ...
	 */
    sbyte classifyPairContent(BracketPair pairedLocation, sbyte dirEmbed)
    {
        sbyte dirOpposite = BidiReference.ON;
        for (int ich = pairedLocation.getOpener() + 1; ich < pairedLocation
                .getCloser(); ich++)
        {
            sbyte dir = getStrongTypeN0(ich);
	 ...
	 */
    sbyte classBeforePair(BracketPair pairedLocation)
    {
	 ...
	 *            - the embedding direction
	 */
    void assignBracketType(BracketPair pairedLocation, sbyte dirEmbed)
    {
        // rule "N0, a", inspect contents of pair
        sbyte dirPair = classifyPairContent(pairedLocation, dirEmbed);
    ...
    private void setBracketsToType(BracketPair pairedLocation, sbyte dirPair)
    {
    ...
    public void resolveBrackets(sbyte dirEmbed)
    {
        foreach (BracketPair pair in pairPositions)
        {
	 ...
	 *            original string 
	 * @param pairValues
	 *            - array of unique ints identifying which pair of brackets 
	 ...
	 *            - the embedding level
	 */
    public void resolvePairedBrackets(int[] indexes, sbyte[] initialCodes, sbyte[] codes, sbyte[] pairTypes,
            int[] pairValues, sbyte sos, sbyte level)
    {
        sbyte dirEmbed = 1 == (level & 1) ? BidiReference.R : BidiReference.L;
        this.sos = sos;
	 ...
	 * @param pairValues
	 *            - array of unique ints identifying which bracket pair
	 ...
	 *            - the embedding level
	 */
    public void runAlgorithm(sbyte[] codes, sbyte[] pairTypes,
            int[] pairValues, sbyte sos, sbyte level)
    {
